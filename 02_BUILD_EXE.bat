...
if exist "template.xlsx" copy /Y "template.xlsx" "dist\RosreestrParser\template.xlsx" >nul

if exist "docs" (
  if exist "dist\RosreestrParser\docs" rmdir /s /q "dist\RosreestrParser\docs"
  xcopy /E /I /Y "docs" "dist\RosreestrParser\docs" >nul
)

REM Prefer config.example.json for public release, but fallback to config.json if example was deleted.
if exist "config.example.json" (
  copy /Y "config.example.json" "dist\RosreestrParser\config.json" >nul
) else (
  if exist "config.json" (
    copy /Y "config.json" "dist\RosreestrParser\config.json" >nul
  )
)
